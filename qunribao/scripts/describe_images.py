...
# ── 描述策略 prompt（含日报/知识卡片特殊识别）──
UNIFIED_PROMPT = """分析这张图片，按以下策略描述：

特殊类型（优先判断，必须严格匹配特征）：
- 群日报/群资源等报告截图：仅当图片标题明确包含「群日报」「群资源」「群周报」字样且带有日期格式（YYYY-MM-DD）时才判定为此类型。判定后只需输出"群日报图片，日期：YYYY-MM-DD"。不要描述报告内容。如果标题不含上述关键词，绝对不能判定为此类型，必须按常规类型处理。
- 知识卡片/信息图：还原所有文字，说明逻辑结构

常规类型：
1. 纯文本截图：逐字还原所有可见文字，不要省略任何内容。即使文字排版像报告、卡片或信息图，只要没有明确的「群日报」标题，都按纯文本截图处理。
2. 操作界面截图：描述布局，还原界面文字
3. 海报/公告：还原标题、时间、地点等所有文字
4. 环境照片：简述场景，提取可见文字
5. 表情包/梗图：简述内容和含义，还原文字
6. 纯表情/贴图（无文字或极少文字的夸张表情图片）：只需用一句话描述传达的情绪或氛围，如"得意""无语""鼓励""惊讶"等，不要描述画面细节
7. 其他：说明图片性质，还原关键内容

请先判断类型，再按对应策略输出。
重要：纯文本截图类型是最安全的默认选择。当不确定图片是否为群日报时，优先按纯文本截图处理，还原所有文字。
重要：输出不要使用任何markdown格式（不要用#、**、-等标记），不要使用中文引号。"""
